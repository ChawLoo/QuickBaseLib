...
fun Context.longToast(message: CharSequence?) = message?.takeIf { it.isNotBlank() }?.let { Toaster.showLong(it) }
fun Context.longToast(@StringRes message: Int) = Toaster.showLong(message)
...
fun Fragment.longToast(t: Throwable?) = t?.message?.takeIf { it.isNotBlank() }?.let { requireActivity().longToast(it) }
fun Context.longToast(t: Throwable?) = t?.message?.takeIf { it.isNotBlank() }?.let { Toaster.showLong(it) }
